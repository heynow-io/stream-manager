...
public class StreamServiceTest extends TestsBase {
    ...
    public void shouldCreateStreamAndObservables() {
        ...
        Node merger = loadedStream.getRootNode().getParents().stream().findFirst().get();
        ...
        Node filter = merger.getParents().stream().filter(node -> node.getOperator().getName().equals("filter")).findAny().get();
        ...
        Node producer1 = filter.getParents().stream().findFirst().get();
        ...
        Node mapper = merger.getParents().stream().filter(node -> node.getOperator().getName().equals("mapper")).findFirst().get();
        assertThat(mapper.getParents()).hasSize(1);
        Node buffer = mapper.getParents().stream().findFirst().get();
        ...
        Node producer2 = buffer.getParents().stream().findFirst().get();
